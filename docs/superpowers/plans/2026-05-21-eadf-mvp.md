...
- **Solidity NatSpec convention:** The contract snippets in Phase 1 below show core logic only. When creating real `.sol` files, follow the full project NatSpec style — `@title`, `@notice`, `@dev` with numbered `SECURITY FEATURES:` / `VULNERABILITIES:` / `DESIGN NOTES:` blocks, per-function and per-state-variable `@notice`, `{CrossReferenced}` links. See [`src/secure/SecureLogicV1.sol`](../../../src/secure/SecureLogicV1.sol) for the canonical example, or look at how Phase 1 commits ended up rendering this in [`src/vulnerable/VulnerableUUPS.sol`](../../../src/vulnerable/VulnerableUUPS.sol). The plan's snippet is a starting point, not the final NatSpec.
- **Python project conventions:** No equivalent NatSpec requirement, but module-level docstrings should cite the spec/SPEC sections they implement (look at existing `eadf/eadf/config.py`, `models.py` for the pattern).
...
contract VulnerableUUPS is Initializable, UUPSUpgradeable {
    ...
    // OZ v5: UUPSUpgradeable is @custom:stateless — no __UUPSUpgradeable_init() exists.
...
**Files:**
- Create: `src/secure/SecureUUPS.sol`
...
```bash
git add src/secure/SecureUUPS.sol
...
# Ship the semantic_tables.toml (and any other data files) inside the wheel
# so importlib.resources finds them after pip install. Without this, eadf/data/
# would be missing from non-editable installs and semantic_scorer would crash.
eadf = ["data/*.toml"]
```
...
eadf collect \
  --local-v1 ../src/vulnerable/VulnerableLogicV1.sol \
  --local-v2 ../src/vulnerable/VulnerableLogicV2.sol \
  --run-id smoke1
```

Expected: `work/smoke1/stage1_sources/v1/VulnerableLogicV1.sol` and `v2/VulnerableLogicV2.sol` exist, plus `metadata.json`.
...
- [ ] **Step 1: Write tests for the diff function** (covers patched §9.2 with slot-0 promotion)
...
def test_high_collision_when_balance_overwritten_not_at_slot_0():
    ...
    m1 = compute_slot_mapping([_sv("x", "uint256"), _sv("balances", "mapping(address => uint256)")])
    m2 = compute_slot_mapping([_sv("x", "uint256"), _sv("config", "mapping(address => uint256)")])
    diff = diff_slot_maps(m1, m2)
...
def test_slot_0_uint256_collision_is_critical():
    ...
    m1 = compute_slot_mapping([_sv("value", "uint256")])
    m2 = compute_slot_mapping([_sv("collisionVar", "uint256")])
    diff = diff_slot_maps(m1, m2)
    assert diff.collisions[0].severity == "Critical"
    assert diff.collisions[0].slot == 0
...
def test_slot_0_bool_collision_is_high():
    ...
    m1 = compute_slot_mapping([_sv("ready", "bool")])
    m2 = compute_slot_mapping([_sv("flag", "bool")])
    diff = diff_slot_maps(m1, m2)
...
def test_medium_collision_for_generic_value_at_non_zero_slot():
    ...
    m1 = compute_slot_mapping([_sv("a", "uint256"), _sv("counter", "uint256")])
    m2 = compute_slot_mapping([_sv("a", "uint256"), _sv("renamedCounter", "uint256")])
    diff = diff_slot_maps(m1, m2)
    coll = next(c for c in diff.collisions if c.slot == 1)
...
- [ ] **Step 4: Create `slot_mapper_severity.py`** (implements patched spec §9.2 with slot-0 promotion)
...
```bash
eadf collect --local-v1 ../src/vulnerable/VulnerableLogicV1.sol --local-v2 ../src/vulnerable/VulnerableLogicV2.sol --run-id smoke2
...
- [ ] **Step 1: Write `semantic_tables.toml`** (lookup tables for F2/F4/F5/F6) — placed **inside** the package so `importlib.resources` finds it after `pip install`

```toml
# eadf/eadf/data/semantic_tables.toml
...
```bash
git add eadf/eadf/module4_matcher/semantic_scorer.py eadf/eadf/data/semantic_tables.toml eadf/tests/test_semantic_scorer.py
...
- [ ] `eadf run --local-v1 src/vulnerable/VulnerableLogicV1.sol --local-v2 src/vulnerable/VulnerableLogicV2.sol --run-id demo` succeeds without crashing and yields `work/demo/stage5_report.json` with `risk_level: "Critical"` and `work/demo/checklist.md` with 🔴 CRITICAL
...
- [ ] Manual:
    - `eadf run --local-v1 ../src/vulnerable/VulnerableLogicV1.sol --local-v2 ../src/vulnerable/VulnerableLogicV2.sol --run-id final` → `risk_level: "Critical"`
