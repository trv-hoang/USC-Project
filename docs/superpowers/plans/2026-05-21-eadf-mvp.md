...
- [ ] **Step 5: Wire into `eadf detect`** — append the call alongside `detect_storage_collision`:

In `cli.py`'s `detect` body, after `run_slither(v1_files[0])`/`run_slither(v2_files[0])`, before assembling `payload`:
...
(Add the `build_ast` import to the `detect` function's local imports.)
