...
- **Uninitialized E2E:** `VulnerableLogicV1` vs `SecureLogicV1` → `missing-disable-initializers` detected in V1 AND NOT detected in V2 (the testable invariant for the detector gap being closed). Note: `upgrade_behavior` in the report ends up `Invalid Upgrade` rather than `Fix Vulnerability` because `SecureLogicV1` introduces OZ Upgradeable inheritance with a different storage layout, producing confounding `storage-collision-cross-version` and `missing-upgrade-authorization` findings on V2. This is a fixture-pair artifact (V1 and V2 are not a clean refactor of each other) rather than a detector bug, and the test asserts only the detector-level invariant.
